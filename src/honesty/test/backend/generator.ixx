...
namespace std
{
	...
	public:
		...
		template<class _This, class _Alloc2, class... _Args>
			requires convertible_to<const _Alloc2&, _Allocator>
		static void*
			operator new(const size_t _Size, const _This&, allocator_arg_t, const _Alloc2& _Al, const _Args&...)
		{
			return _Allocate(static_cast<_Alloc>(static_cast<_Allocator>(_Al)), _Size);
		}
	...
	private:
		...
		struct _Nested_awaitable
		{
			...
			template<class _Promise>
			[[nodiscard]] coroutine_handle<_Gen_promise_base>
	...
	public:
...
#ifdef __cpp_lib_is_pointer_interconvertible
		static_assert(is_pointer_interconvertible_base_of_v<_Gen_promise_base<_Gen_yield_t<_Ref>>, promise_type>);
#endif	// __cpp_lib_is_pointer_interconvertible
		...
		[[nodiscard]] _Gen_iter<_Value, _Ref> begin()
		{
			...
			return _Gen_iter<_Value, _Ref> {
				_Gen_secret_tag {},
				coroutine_handle<_Gen_promise_base<_Gen_yield_t<_Ref>>>::from_address(_Coro.address())};
		}

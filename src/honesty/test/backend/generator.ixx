...
namespace synodic::honesty
{
	...
	using _Rebind = typename std::allocator_traits<_Alloc>::template rebind_alloc<_Aligned_block>;

	template<class _Alloc>
	concept _Has_real_pointers =
		std::same_as<_Alloc, void> || std::is_pointer_v<typename std::allocator_traits<_Alloc>::pointer>;
	...
	public:
		...
		static void* operator new(const size_t _Size, std::allocator_arg_t, const _Alloc2& _Al, const _Args&...)
		{
		...
		template<class _This, class _Alloc2, class... _Args>
			requires std::convertible_to<const _Alloc2&, _Allocator>
		static void*
			operator new(const size_t _Size, const _This&, std::allocator_arg_t, const _Alloc2& _Al, const _Args&...)
		{
			return _Allocate(static_cast<_Alloc>(static_cast<_Allocator>(_Al)), _Size);
		}
		...
		static void* _Allocate(const _ProtoAlloc& _Proto, size_t _Size)
		{
			...
			if constexpr (std::default_initializable<_Alloc> && std::allocator_traits<_Alloc>::is_always_equal::value)
			{
				...
				const size_t _Count =
					(_Size + sizeof(_Dealloc_fn) + sizeof(_Aligned_block) - 1) / sizeof(_Aligned_block);
				void* const _Ptr = _Al.allocate(_Count);
				std::memcpy(static_cast<char*>(_Ptr) + _Size, &_Dealloc, sizeof(_Dealloc));
				return _Ptr;
			}
			else
			{
	...
	public:
		...
		static void* operator new(const size_t _Size, std::allocator_arg_t, const _Alloc& _Al, const _Args&...)
		{
		...
		static void*
			operator new(const size_t _Size, const _This&, std::allocator_arg_t, const _Alloc& _Al, const _Args&...)
		{
			static_assert(_Has_real_pointers<_Alloc>, "coroutine allocators must use true pointers");
			return _Allocate(_Al, _Size);
		}
		...
		static void operator delete(void* const _Ptr, const size_t _Size) noexcept
		{
			...
			std::memcpy(&_Dealloc, static_cast<const char*>(_Ptr) + _Size, sizeof(_Dealloc_fn));
			_Dealloc(_Ptr, _Size);
		}
	};
	...
	template<class _Rty, class _Vty>
	using _Gen_reference_t = std::conditional_t<std::is_void_v<_Vty>, _Rty&&, _Rty>;
	template<class _Ref>
	using _Gen_yield_t = std::conditional_t<std::is_reference_v<_Ref>, _Ref, const _Ref&>;
	...
	private:
		...
		struct _Final_awaiter
		{
			...
			[[nodiscard]] std::coroutine_handle<> await_suspend(std::coroutine_handle<_Promise> _Handle) noexcept
			{
				static_assert(std::is_pointer_interconvertible_base_of_v<_Gen_promise_base, _Promise>);
				...
				std::coroutine_handle<_Gen_promise_base> _Cont = _Current._Info->_Parent;
		...
		{
			static_assert(std::same_as<_Gen_yield_t<_Gen_reference_t<_Rty, _Vty>>, _Yielded>);
				...
				await_suspend(std::coroutine_handle<_Promise> _Current) noexcept
			{
				...
				auto _Target	= std::coroutine_handle<_Gen_promise_base>::from_address(_Gen._Coro.address());
				_Nested._Parent = std::coroutine_handle<_Gen_promise_base>::from_address(_Current.address());
	...
	template<class _Rty, class _Vty, class _Alloc>
	class generator : public std::ranges::view_interface<generator<_Rty, _Vty, _Alloc>>
	{
	private:
		using _Value = _Gen_value_t<_Rty, _Vty>;
		static_assert(
			std::same_as<std::remove_cvref_t<_Value>, _Value> && std::is_object_v<_Value>,
	...
	public:
		struct EMPTY_BASES promise_type : _Promise_allocator<_Alloc>, _Gen_promise_base<_Gen_yield_t<_Ref>>
		{
			[[nodiscard]] generator get_return_object() noexcept
			{
				return generator {_Gen_secret_tag {}, std::coroutine_handle<promise_type>::from_promise(*this)};
			}
		};

		static_assert(std::is_standard_layout_v<promise_type>);
		static_assert(std::is_pointer_interconvertible_base_of_v<_Gen_promise_base<_Gen_yield_t<_Ref>>, promise_type>);
		...
		[[nodiscard]] _Gen_iter<_Value, _Ref> begin()
		{
			...
			return _Gen_iter<_Value, _Ref> {
				_Gen_secret_tag {},
				std::coroutine_handle<_Gen_promise_base<_Gen_yield_t<_Ref>>>::from_address(_Coro.address())};
		}
	...
	private:
		...
		explicit generator(_Gen_secret_tag, std::coroutine_handle<promise_type> _Coro_) noexcept :
			_Coro(_Coro_)
		{
		}
	};
